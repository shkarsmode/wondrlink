...
export class VoicesComponent {
  ...
  constructor() {
    // auto-fetch при смене фильтров/страницы/лимита
    effect(() => {
      const p = this.page();
      const l = this.limit();
  ...
  public async onStatusAction(v: IVoice, next: VoiceStatus) {
    ...
    try {
      ...
      // локально обновим, чтобы не дёргать весь список
  ...
  public async onDelete(v: IVoice) {
    if (!confirm(`Delete submission #${v.id}? This cannot be undone.`)) return;
    try {
      this.loading.set(true);
